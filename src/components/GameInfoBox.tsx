...
const GameInfo: React.FC<GameInfoProps> = ({ reference, showInfo, setShowInfo }) => {
  const steps: string[] = [
    "Press Start to start the game.",
    "After a few seconds the game will start.",
    "When a color Box flicker remember the box and click on it.",
    ...
    "If your answer is wrong then the start button appears again, and heading changes to 'Game Over start Again'"
  ];
            ...
            <div className="relative w-[80vw] mx-auto max-w-5xl">
              ...
              <div className="border-0  rounded-xl shadow-lg relative flex flex-col w-full bg-[#0e2d4d] text-[#fef2bf] outline-none focus:outline-none">
                ...
                <div className="flex items-center justify-between py-3 lg:py-5 px-5 lg:px-7 border-b-2 border-solid border-slate-200 rounded-t">
                  ...
                  <div className="material-symbols-outlined text-gray-400 hover:text-red-500 cursor-pointer font-extrabold" onClick={handleClose}>
                    close
                  </div>
                </div>
                {/* body */}
                ...
                <div className="bg-[#011F3F] m-2 mt-10 h-10 rounded-lg text-center items-center flex justify-center">
                  All rights reserved &copy; Cook Heng
                </div>
              </div>
            </div>
          </div>
...
// Define interface for ListItem props
